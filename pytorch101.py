...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    if stop - start < 10:
        x = torch.tensor( (), dtype=torch.float64).new_zeros((0,))
        return x
...
def slice_assignment_practice(x: Tensor) -> Tensor:
    ...
    x[0:2,0:6] = torch.tensor([[0,1,2,2,2,2]])
    x[2:4,0:6] = torch.tensor([[3,4,3,4,5,5]])
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    M = x.shape[0]
    idx = torch.tensor([M-1])
    #make reverse tensor LMAO HVORFOR VAR DET HER SÅ SVÆRT
    for i in range(M):
        if M-i-2 < 0:
            break
        tmp = torch.tensor([M-i-2])
        idx = torch.cat((idx, tmp), 0)
